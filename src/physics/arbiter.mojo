...
struct FeaturePair(CollectionElement):
    ...
    fn __getitem__(inout self) -> ref [__origin_of(self.e)] Edges:
...
struct Arbiter(CollectionElement):
    ...
    fn update(
            ...
            # inout new_contacts: Tuple[Contact, Contact],
            num_new_contacts: Int32,
            world_warm_start: Bool):
        var merged_contacts = InlineArray[Contact, Self.MAX_POINTS](Contact())

        for i in range(int(num_new_contacts)):
            var c_new = Pointer.address_of(new_contacts[i])

            var k: Int = -1
            for j in range(int(self.num_contacts)):

                var c_old = Pointer.address_of(self.contacts[j])
            ...
            if k > -1:
                var c = Pointer.address_of(merged_contacts[i])
                var c_old = Pointer.address_of(self.contacts[k])
                c[] = c_new[]
    ...
    fn pre_step(inout self, inv_dt: Float32, world_pos_cor: Bool, accumulate_impulses: Bool):
        ...
        var b1 = Pointer.address_of(self.b1[])
        var b2 = Pointer.address_of(self.b2[])

        for i in range(int(self.num_contacts)):

            var c = Pointer.address_of(self.contacts[i])

            var r1: Vector2 = c[].position - b1[].position
            var r2: Vector2 = c[].position - b2[].position
            ...
            var tangent: Vector2 = cross(c[].normal, 1.0)
            ...
            if accumulate_impulses:
                ...
                var P: Vector2 = (c[].normal * c[].Pn) + (tangent * c[].Pt)
