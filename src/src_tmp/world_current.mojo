...
struct World[screen_width: Int, screen_height: Int, *component_types: CollectionElement]:
    ...
    fn _add_default_archetype(inout self):
        self.archetype_index[hash(Self.default_type)] = Archetype()
    ...
    fn add_entity[*Ts: CollectionElement](inout self, *components: *Ts) raises -> Entity:
        ...
        var row = self.archetype_index[hash(Self.default_type)].add_entity()

        # add the entity to the entity index
        self.entity_index[entity] = EntityRecord(UnsafePointer.address_of(self.archetype_index[hash(Self.default_type)]), row)
