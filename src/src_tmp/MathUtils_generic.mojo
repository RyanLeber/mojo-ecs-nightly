...
@register_passable
struct Vector2[type: DType = DType.float32](Absable):
    var data: SIMD[type, 2]

    @always_inline
    fn __init__(inout self, x: Scalar[type], y: Scalar[type]):
        self.data = SIMD[type, 2](x, y)
    ...
    # @staticmethod
    # fn __is__[lhs: CollectionElement]:

    @always_inline
    fn __init__(inout self, data: SIMD[type, 2]):
        self.data = data
    ...
    @always_inline
    fn __setitem__(inout self, idx: Int, value: Scalar[type]):
        self.data[idx] = value

    @always_inline
    fn __setattr__[name: StringLiteral](inout self, val: Scalar[type]):
        @parameter
        if name == "x":
            self.data[0] = val
        elif name == "y":
    ...
    @always_inline
    fn __getattr__[name: StringLiteral](borrowed self) -> Scalar[type]:
        @parameter
        if name == "x":
            return self.data[0]
        elif name == "y":
    ...
    @always_inline
    fn __matmul__(self, other: Self) -> Scalar[type]:
        return (self.data * other.data).reduce_add()

    @always_inline
    fn __mul__(self, k: Float64) -> Self:
        return self.data * k.cast[type]()

    @always_inline
    fn __mul__(v: Self, m: Mat22[type]) -> Self:
    ...
    fn rotate(self, radians: Float64) -> Vector2[type]:
        var s = sin(radians)
        var c = cos(radians)

        var x = c * self.x.cast[DType.float64]() - s * self.y.cast[DType.float64]()
        var y = s * self.x.cast[DType.float64]() + c * self.y.cast[DType.float64]()
        return Vector2(x.cast[type](), y.cast[type]())
...
@value
@register_passable
struct Mat22[type: DType = DType.float32](Absable):
    var col1: Vector2[type]
    var col2: Vector2[type]

    @always_inline
    fn __init__(inout self):
        self.col1 = Vector2[type](0, 0)
        self.col2 = Vector2[type](0, 0)
        
    @always_inline
    fn __init__(inout self, col1: Vector2[type], col2: Vector2[type]):
        self.col1 = col1
        self.col2 = col2

    @always_inline
    fn __init__(inout self, angle: Float64):
        var c = cos(angle).cast[type]()
        var s = sin(angle).cast[type]()

        self.col1 = Vector2(c, s)
        self.col2 = Vector2(-s, c)

    @always_inline
    fn __mul__(A: Self, v: Vector2[type]) -> Vector2[type]:
...
@always_inline
fn dot[type: DType](a: Vector2[type], b: Vector2[type]) -> Scalar[type]:
...
@always_inline
fn cross[type: DType](v: Vector2[type], s: Scalar[type]) -> Vector2[type]:
    return Vector2(s * v.y, -s * v.x)

@always_inline
fn cross[type: DType](s: Scalar[type], v: Vector2[type]) -> Vector2[type]:
    return Vector2(-s * v.y, s * v.x)

@always_inline
fn scalar_vec_mul[type: DType](s: Scalar[type], v: Vector2[type]) -> Vector2[type]:
    return v * s
